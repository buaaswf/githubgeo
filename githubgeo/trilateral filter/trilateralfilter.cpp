...
Raw3D::~Raw3D(void){
	 cout<<"Raw3D is out"<<endl;
}
//added by me
Raw2D* rawarray(int xsize,int ysize,int const zsize,PIXTYPE *y)
{
	int i=0;
	...
	for(i;i<zsize;i++)
	{	p+=zsize;
	raw2D[i]=new Raw2D(xsize,ysize,p);
	raw2D[i]->TrilateralFilter(raw2D[i],20);
	printf("no  %d\n" ,i);
	//raw2D++;
	}
	return raw2D[zsize];
